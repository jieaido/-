...
namespace WindowsFormsApplication1
{
    public  static class NyData
    {
        ...
        public static IEnumerable<NyModel> Getdata(int year,int month)
        {
            ...
            {
                string cmd = "select TagName, sum(Val_) as sumval from " +
                         "(select TagName,Val_,DateAndTime from FloatTable where DateAndTime between @date1 and @date2)  " +
        ...
        private static Dictionary<string, string> tagnameDict = new Dictionary<string, string>()
        {
            {"[JSWS]AI_01", "压力1"},
            {"[JSWS]AI_02", "压力2"},
            {"[JSWS]FIT_01", "流量1"},
